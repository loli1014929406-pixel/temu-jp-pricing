-- Exact finance order metrics used by paged detail and aggregate RPCs.
-- Functions are SECURITY INVOKER so existing table RLS remains authoritative.
...
declare
  ...
  v_currency text := coalesce(v_params->>'currency', case when v_formula = 'flat_jpy' then 'JPY' else 'RMB' end);
  ...
  with settings as (
    ...
    select mo.*, coalesce(ws.has_first, false) as has_first, coalesce(ws.has_last, false) as has_last,
      ...
      coalesce(public.finance_dynamic_method_cost(lm.config,
        ...
        s.exchange_rate_rmb_per_jpy), 0) as estimated_last_cost
    ...
    select wl.*, coalesce(st.sales_revenue,0) as sales_revenue,
      ...
      case when wl.actual_shipping_fee_rmb > 0 then 'actual'
        when wl.estimated_last_cost > 0 then 'estimated' else 'missing' end as fee_source
    from with_logistics wl
    left join settlements st on st.po_number = btrim(wl.order_no)
  )
  select
    to_jsonb(c) - array['sku_json','product_json','metric_date','sku_id','product_id','unit_cost','product_code','product_name_cn','package_weight_g','has_first','has_last','first_cost','sales_revenue','freight_revenue','settled','logistics_issue','fee_source'],
    c.sku_json,
    c.product_json,
    c.metric_date,
    ...
    round(case when c.actual_shipping_fee_rmb > 0 then c.actual_shipping_fee_rmb else c.estimated_last_cost end,2),
    round((case when c.logistics_issue = '' then c.first_cost else 0 end)
      + (case when c.actual_shipping_fee_rmb > 0 then c.actual_shipping_fee_rmb else c.estimated_last_cost end),2),
    c.fee_source,
    c.logistics_issue,
    c.sales_revenue,
    c.freight_revenue,
