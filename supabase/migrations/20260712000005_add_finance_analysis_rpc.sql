-- Paged finance order details plus database-side summaries from the shared exact metric function.
...
as $$
  with metrics as (
    ...
    where (p_date_start is null or finance_date >= p_date_start)
      ...
      and (btrim(coalesce(p_search,'')) = '' or lower(
        ...
        tracking_no || ' ' || recipient_name || ' ' || product_code || ' ' || product_name
    ...
    select case when btrim(logistics_method) = '' then '未填写发货方式' else regexp_replace(btrim(logistics_method),'\s+',' ','g') end as method,
      count(*) as order_count, round(sum(quantity),2) as quantity,
  ...
  select
    ...
    coalesce((select jsonb_agg(to_jsonb(shipping_rows) order by total_shipping desc, order_count desc) from shipping_rows),'[]'::jsonb);
$$;
