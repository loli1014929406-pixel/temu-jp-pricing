-- Split warehouse / shipping-method estimates into first-leg and last-leg totals.
-- Existing actual_shipping and estimated_shipping keys are retained for compatibility.
...
as $$
  with metrics as (
    select m.*,
      ...
      coalesce(nullif(m.product_data->>'package_weight_g','')::numeric,0) as package_weight_g,
    ...
    select
      ...
      round(sum(greatest(package_weight_g,0) * greatest(quantity,0)),2) as weight_g,
      round(sum(first_leg_shipping_rmb),2) as first_leg_shipping,
      round(sum(case when shipping_fee_source='actual' then last_leg_shipping_rmb else 0 end),2) as actual_last_leg_shipping,
      round(sum(case when shipping_fee_source='estimated' then last_leg_shipping_rmb else 0 end),2) as estimated_last_leg_shipping,
      round(sum(last_leg_shipping_rmb),2) as last_leg_shipping,
      round(sum(case when shipping_fee_source='actual' then last_leg_shipping_rmb else 0 end),2) as actual_shipping,
      round(sum(first_leg_shipping_rmb) + sum(case when shipping_fee_source='estimated' then last_leg_shipping_rmb else 0 end),2) as estimated_shipping,
