...
async function proxyYamato(request: Request, env: Env) {
...
async function proxyJapanPost(request: Request) {
  ...
  try {
    ...
    console.error("Japan Post tracking proxy failed", error);
...
export default {
  async fetch(request, env): Promise<Response> {
    ...
    if (pathname === japanPostTrackingPath) return proxyJapanPost(request);
