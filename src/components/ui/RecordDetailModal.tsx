...
export function RecordDetailModal({
  ...
  return (
    <div
      ...
      aria-label={title}
    >
      <div className={`grid max-h-[88vh] w-full ${maxWidthClassName} grid-rows-[auto_minmax(0,1fr)] overflow-hidden rounded-xl border border-[#e3e3e3] bg-white shadow-xl`}>
          ...
          {rows.length > 0 && (
            <dl className="grid grid-cols-1 gap-3 sm:grid-cols-2">
              {rows.map((row, index) => (
                <div
                  key={`${row.label}-${index}`}
                  className={`rounded-lg border border-[#e3e3e3] bg-[#f7f7f7] px-3 py-2 ${row.wide ? "sm:col-span-2" : ""}`}
                >
